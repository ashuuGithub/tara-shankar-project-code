...
class Benevity(FileLoader):
    ...
    def trim(self):
        ...
        cursor = conn.cursor()
        try:
            self.log.info(f'Trimming transactions on TRUST.{self.name}')
            cursor.execute(f'TRUNCATE TABLE TRUST.{self.name}')  # Benevity will always be loaded, so truncate the table every time
            conn.commit()
            self.log.info(f'Completed trimming transactions on TRUST.{self.name}')
        finally:
            cursor.close()
            conn.close()
