...
class Benevity(FileLoader):
    ...
    def is_file_processed(self, file_name):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("SELECT 1 FROM Benevity_Processed_Files WHERE FileName = ?", (file_name,))
    ...
    def mark_file_as_processed(self, file_name):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("INSERT INTO Benevity_Processed_Files (FileName) VALUES (?, ?)", (file_name))
            conn.commit()
        finally:
            cursor.close()
            conn.close()
    ...
    def process_file(self, file_path, file_name, file_date, start_date):
        ...
        try:
            ...
            tuple[]
            # Load the Excel file and find the correct sheet
            ...
            df = df[['COMPANY', 'PROJECT', 'DONATIONDATE', 'FIRSTNAME', 'LASTNAME', 'EMAIL', 'ADDRESS', 
                     'CITY', 'STATECODE', 'ZIPCODE', 'ACTIVITY', 'COMMENT', 'TRANSACTIONID', 
                     'DONATIONFREQUENCY', 'CURRENCY', 'PROJECTREMOTEID', 'SOURCE', 'REASON', 
                     'TOTALDONATIONTOBEACKNOWLEDGED', 'MATCHAMOUNT', 'CAUSESUPPORTFEE', 
                     'MERCHANT_FEE', 'FEECOMMENT']]

            # Convert dates and other necessary columns to the correct data types
            ...
            self.log.info(f"Finished Benevity File with {len(tuples)} records.")
